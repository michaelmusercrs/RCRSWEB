...
- `[done]` **1.5 Per-recipient rate limit (proper)** — `lib/email-service.ts:87-232` now KV-backs the per-recipient cap (`email-bucket:{addr}:hour:{H}` / `:day:{D}` keys, INCR+EXPIRE bucket-per-window). Falls back to legacy in-memory Map if `KV_REST_API_URL`/`KV_REST_API_TOKEN` are absent or `@vercel/kv` import fails — so `npm run dev` still works. Hour cap 5/owner-gmail or 30/other, day cap 30/100, both env-overridable. Closes the Vercel cold-start bypass that made the 5/hr owner-cap effectively 5×N (warm instances).
- `[done]` **1.6 Resend transport integration** — `lib/email-service.ts` swapped from Google Apps Script `fetch()` to the Resend SDK. Template allowlist gates sends — default allowlist = `contact-form`, `load-verified-invoice`, `driver-new-order` (per owner directive 2026-05-20). Untagged sends drop with `[EMAIL TEMPLATE NOT ALLOWED]` log.
- `[done]` **1.7 Resend provisioned + LIVE (2026-07-02)** — owner created the account; sender domain **rcrsal.com** verified (DKIM+SPF via Vercel DNS, ~1 min); `RESEND_API_KEY` + `EMAIL_FROM=noreply@rcrsal.com` set on rcrs-portal production; live test send delivered to owner gmail. Note: free plan = 1 domain; a typo'd `riverityroofingsolutions.com` (missing c) was deleted from Resend. Public-brand sender (rivercityroofingsolutions.com) deferred — its DNS lives in Squarespace (Google nameservers), records must be added there; only needed when customer-facing email is approved (see feedback_no_customer_emails_until_approval).
...
- `[done]` **2.6b Migrate the 10 form endpoints from `lib/rate-limiter` to `lib/rate-limiter-kv`** — superseded by the lower-down 2.6b done entry; migration completed with per-route factories + global 15/hr IP cap.
...
- `[done]` **3.8 HIGH: Cost-visibility drift** — owner decided 2026-07-02: PMs do NOT see cost. Removed `project_manager`/`pm` from `lib/cost-visibility.ts` allowlist; Office granted `inventory.viewCosts` in `permissions.ts`; both command-center inventory pages' `showCost` gates now owner/admin/office/manager (PMs keep selling-price visibility only).
...
- `[done]` **3b.4 Portal cost-data role-tiering** — new `requireRoleAtLeast(roles, allowedSlugs?)` helper in `lib/auth-service.ts` with Richard's `driver` slug whitelisted (cost-visibility allowlist per memory). 4 named routes + 4 ADDITIONAL routes tightened (8 total): reports/commissions, reports/profitability, reports/finance-summary, jobnimbus/commissions, meeting-data, transactions (unitCost + profitMargin), credit-memos GET (unitCost), delivery/job-breakdowns (totalCost + profit). Left alone where intentional (my-commissions = rep's own stats; customer-breakdowns has internal redactAdminTier; portal/tickets driver workflow).
- `[done]` **3b.7 Cost-privacy guard on JobNimbus reads** — `lib/jn-redact.ts` defines the canonical `COST_FIELD_NAMES` list. ~23 read functions in `lib/jobnimbus-service.ts` + 7 other lib service files + 16 API route callsites updated to take an optional `viewer` arg and call `redactCostFieldsDeep(raw, viewer?.canSeeCost ?? false)` before returning. Default fail-safe (omitted viewer = no-cost view). Cache keys include `canSeeCost` so no cross-tier leak via cache. CRM moat win #3.

## Phase 7 — Feature work on sweep branches (continued)

- `[done]` **7.6 Monday meeting prep auto-fill (sweep)** — `sweep/monday-prep-autofill`. `lib/monday-prep-autofill.ts` aggregates traceable-source figures (3 leaderboards never combined per memory rule), confidence-scored announcement proposals (<0.7 = owner-review badge), Open-Meteo weather callout. UI at `/command-center/meetings/prep-autofill`. Sunday-6pm-Central pre-gen cron parked in `vercel.json _disabledCrons`. Sheet-backed save on `Monday Prep` tab, idempotent on weekStart. CRM moat win #2 — saves ~30 min every Monday.
...
## Phase 9 — Live inventory / job-flow / portal / phone

- `[done]` **9.1 Catch-up invoice batch (25 invoices, $41,648.06)** — 25 delivery tickets that had been loaded + delivered but never invoiced. Pipeline: read master sheet → filter real (vs test) → look up materials with prices → write 25 rows to `Invoices` tab → deduct `Inventory_Products` per material → write 25 `Delivery Schedule` rows with status (delivered/loaded/pending based on completedAt vs requestedDate) → tag source tickets on `Tickets` tab. ONE Gmail draft to stock@rcrsal.com in owner inbox with full breakdown table — owner clicks send. Scripts at `scripts/{read-pending-tickets,list-pending-invoices,generate-invoice-drafts,post-invoices-and-deduct,post-invoices-resume}.mjs`. Idempotent; safe to re-run.
- `[done]` **9.2 Backfill ALL remaining historical invoices + job breakdowns (no emails)** — owner directive 2026-05-20 "make sure they each have an invoice and job breakdown but don't email them for bulk creation." Result: **234 new invoices** (INV-20260520-0100..0335 range, $268,909.67 grand total) + **264 job breakdowns** (one per invoice via fix-script catch-up) + delivery schedule rows + source tickets tagged. **No inventory deductions for pre-2026-05-15 tickets** (historical_close snapshot already accounted for those — deducting would have double-counted). No emails sent. Scripts: `bulk-backfill-invoices-and-breakdowns.mjs` + `fix-missing-job-breakdowns.mjs`.
- `[done]` **9.3 Inventory app UI: hover help + quick-actions + delivery schedule + training** — shipped to main (commit 2918c82). `components/inventory/{HelpTooltip,InventoryQuickActions}.tsx` + `lib/delivery-schedule-service.ts` + 5 role-tagged training modules wired into existing `getModulesForAuthRole()`.
- `[done]` **9.4 Customer portal audit + team profile self-service (sweep)** — `sweep/profile-self-service`. Audit identifies 3 stubs per memory (service request, warranty claim, notification prefs) + 1 wiring gap (ReviewSubmission). Profile self-service: edits go to PENDING `Team_Profile_Edits` sheet → admin reviews. Reviews_Master tab schema scaffolded. Import endpoint accepts JSON paste from Google/BBB/Yelp/Facebook (each source needs API creds — `[needs-owner]`).
- `[done]` **9.5 Phone system FreePBX integration (sweep)** — `sweep/phone-system`. `lib/freepbx-service.ts` (REST client, env-gated), `components/ClickToCallButton.tsx`, new sub-pages (queues/routing/recordings/numbers), `docs/phone-system-roadmap.md`. Existing portal phone UI updated to live FreePBX status. `[needs-owner]`: WAN reachability (Cloudflare Tunnel?), FreePBX API key generation, DID provider details.
- `[done]` **9.6 Auto-blog 2 bugs fixed + social-publish stub** — commit 22d630a. Bug A: `/api/portal/blog/pending` was reading from local JSON (lost on Vercel). Bug B: rep posts published to `Blog_Posts` tab but `/blog` reads from `blog-posts` tab — cron now mirrors. `lib/social-publish.ts` stub for FB/IG/LinkedIn/X (skipped:true until env). docs/social-blog-audit.md catalogs. Social-media auto-post was NEVER built in repo (owner's recollection likely was Meta Business Suite / Buffer).
...
- `[done]` **10.4 Audit H1 — template tags on 19 untagged callsites** — every ad-hoc `emailService.send()` in the audit now carries a `template:` tag (login-alert, job-status-update, breakdown-draft, notification-dispatch, review-request, careers-application, work-order, storm-report-customer, storm-report-sales, weekly-numbers-reminder, stalled-tickets-digest, low-stock-alert, profile-edit-request, vendor-alert, weekly-numbers-submitted, customer-invoice, new-lead-office, return-credit, pipeline-milestone). `EmailTemplate` union extended in `lib/email-service.ts`. **No behavior change today** — default allowlist still drops them; once owner expands `ALLOWED_EMAIL_TEMPLATES` env on Vercel each callsite enables independently. Clean tsc.
- `[done]` **10.5 Audit H2 — storm-report spam-filter** — `app/api/storm-report/route.ts` was the only public form skipping `checkForSpam` (highest-volume lead-magnet, biggest bot target). Added between honeypot and Turnstile with success-shape (200 OK) on block + `logSpamBlock` entry tagged gate=`spam-filter`. Verified clean tsc.
- `[done]` **10.6 Cron audit Phase 2 — `withCronLock` adoption** — 6 production cron routes wrapped with `withCronLock` from `lib/cron-lock.ts` (built in commit eef7881). Routes + staleMinutes: `sync-inventory-tab` 15, `auto-reassign` 10, `backup-sheets` 60, `check-lead-timers` 5, `publish-blog` 30, `marketing-intel` 60. Pattern: `CRON_SECRET` auth stays outside the lock (unauthorized hits don't cost a sheet round-trip); entire post-auth body — including all `NextResponse.json` returns — lives inside the lock callback. Closes the manual-hit-collides-with-scheduled-hit + slow-overruns-next-slot race risks called out in `docs/cron-audit-2026-05-21.md`. Clean tsc.
...
- `[done]` **7.5 Hail auto-canvass list (sweep)** — `sweep/hail-canvass` branch. Admin UI at `/command-center/canvass` for post-storm rep door-knock list generation. K-means rep clustering with balancing pass. Damage scoring deterministic from hail size + distance + customer-status penalty. Real address data only — pulls from existing `Geocoded_Contacts` sheet (populated by `lib/geocode-sync.ts`). SVG fallback map (no Leaflet dep). Idempotent sheet-backed assignment to a new `Canvass Routes` tab. 3 `[needs-owner]` blockers in code: parcel-GIS expansion (privacy call), proactive NOAA cron, rep notification channel.
- `[done]` **5.6c OG/Twitter cards + opengraph-image** — main. 6 dynamic-route pages + community page upgraded to emit full openGraph + twitter metadata via `lib/seo.ts:generateMetadata()`. 3 dynamic `opengraph-image.tsx` routes (site root, /services/[slug], /roofing-contractor/[county]) using `next/og` ImageResponse — dark `#0b1220` background, `#0066CC` accent, system fonts (Vercel-safe), IKO-first subtitle, phone, edge runtime 1200×630.
...
- `[done]` **6.1 Bootstrap LOG.md** — file present at repo root with the full email-flood incident timeline (10:18 first cap, 10:20 GAS fetch disabled, 10:30 hard-disable, etc.) plus all subsequent sweep actions through Phase 10.3. Reverse-chronological, single-line, with commit refs.
- `[in-progress]` **6.2 Routine appends to LOG.md every run** — convention is established (see header of LOG.md) and routine has been honoring it through Phase 10. Gap: the parallel chrisview-spree worker (commits 110b4a3 → 83f92d3) did NOT add per-commit LOG entries — those have now been backfilled below. Reinforced the routine instruction in this file (step 4 + new explicit "non-optional" line).
...
4. Work on it for up to ~20 min. If finished, mark `[done]` and append a one-line LOG.md entry. If partial, leave at `[in-progress]` with a short note of where you stopped. **LOG.md append is non-optional** — every commit pushed to `main` or a `sweep/*` branch by the routine MUST have a matching single-line LOG entry. If you skip it the owner loses the audit trail and Phase 6.2 regresses.
