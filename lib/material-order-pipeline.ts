...
class MaterialOrderPipelineService {
  ...
  async advanceStage(
    ...
    switch (targetStage) {
      ...
      case 'INVOICE_SENT':
        ...
        if (order.customerInvoiceId) {
          ...
          if (custInv) {
            ...
            // Fire the customer-facing invoice email with PDF attached.
            // Non-fatal on failure (the invoice is already marked sent and
            // persisted) — but we await it so logs are visible before the
            // Vercel lambda exits.
            await this._sendCustomerInvoiceEmail(order, custInv).catch((err) =>
              console.error('[pipeline] INVOICE_SENT customer email failed:', err)
            );
          }
        }
        break;
      ...
      case 'PAYMENT_RECEIVED':
        ...
        {
          // Extract payment metadata up-front so the receipt email gets the
          // method/reference instead of a generic "payment received" note.
          const meta = (data?.metadata || {}) as Record<string, unknown>;
          if (meta.paymentAmount != null) {
            order.paymentAmount = Number(meta.paymentAmount);
          }
          event.metadata = JSON.stringify({
            ...JSON.parse(event.metadata || '{}'),
            paymentAmount: meta.paymentAmount,
            paymentMethod: meta.paymentMethod || 'unknown',
            paymentDate: meta.paymentDate || now,
          ...
          if (order.customerInvoiceId) {
            ...
            if (custInv) {
              ...
              // Fire the receipt email to the customer
              await this._sendPaymentReceiptEmail(order, custInv, {
                amount: Number(meta.paymentAmount || custInv.total),
                method: String(meta.paymentMethod || 'check'),
                reference: meta.paymentReference ? String(meta.paymentReference) : undefined,
                date: String(meta.paymentDate || now),
              }).catch((err) =>
                console.error('[pipeline] PAYMENT_RECEIVED receipt email failed:', err)
              );
            }
          }
  ...
  /**
   * Send the customer-facing invoice PDF email at INVOICE_SENT.
   *
   * PRICE ONLY. Admin (Michael, Chris, Sara) are cc'd so the office has a
   * copy of every customer invoice. The PDF is generated by
   * lib/invoice-pdf.tsx (price-only flavor) and attached via the email
   * service's attachments path. Non-fatal on failure — the invoice is
   * still marked sent and persisted.
   */
  private async _sendCustomerInvoiceEmail(
    order: PipelineOrder,
    invoice: PipelineInvoice
  ): Promise<void> {
    // Need a customer email to send to. If we don't have one, log and skip —
    // not a blocking error.
    const customerEmail = order.customerEmail || invoice.customerEmail;
    if (!customerEmail) {
      console.warn(
        `[pipeline] INVOICE_SENT: no customer email on order ${order.orderId}; PDF not sent.`
      );
      return;
    }
    ...
    if (!emailService.isConfigured()) {
      console.warn('[pipeline] INVOICE_SENT: email service not configured.');
      return;
    }
    ...
    try {
      const { renderCustomerInvoicePDF } = await import('./invoice-pdf');
      pdfBuffer = await renderCustomerInvoicePDF(invoice, order);
    } catch (err) {
      console.error('[pipeline] INVOICE_SENT: PDF render failed:', err);
      return;
    }

    // Admin cc list: Michael, Chris (owners), Sara (admin) — looked up
    // from team-roles so changes there propagate automatically.
    const adminCc = TEAM_MEMBERS
      .filter(m =>
        m.isActive &&
        (m.role === 'owner' || m.role === 'admin' ||
         m.slug === 'michael-muse' || m.slug === 'chris-muse' || m.slug === 'sara-hill')
      )
      .map(m => m.email)
      .filter(Boolean);
    const ccList = Array.from(new Set(adminCc)).join(',');

    const subject = `Invoice ${invoice.invoiceId} — River City Roofing Solutions`;
    ...
    const body = `
      <div style="font-family: Arial, sans-serif; max-width: 640px; margin: 0 auto;">
        <div style="background: #0a0a0a; padding: 18px; text-align: center;">
          <h1 style="color: #39FF14; margin: 0; letter-spacing: 1px;">Invoice ${invoice.invoiceId}</h1>
          <p style="color:#d1d5db;margin:6px 0 0;font-size:13px;">River City Roofing Solutions</p>
        </div>
        <div style="padding: 26px; background: #fff; color:#1f2937;">
          <p>Hi ${invoice.customerName || 'there'},</p>
          <p>
            Thank you for choosing River City Roofing Solutions. Please find your
            invoice attached as a PDF.
          </p>
          <table style="width:100%;border-collapse:collapse;margin:14px 0;">
            <tr><td style="padding:4px 8px;font-weight:bold;">Invoice #</td><td style="padding:4px 8px;">${invoice.invoiceId}</td></tr>
            <tr><td style="padding:4px 8px;font-weight:bold;">Job</td><td style="padding:4px 8px;">${invoice.jobName || invoice.jobNumber || '—'}</td></tr>
            ${jobAddr ? `<tr><td style="padding:4px 8px;font-weight:bold;">Service Address</td><td style="padding:4px 8px;">${jobAddr}</td></tr>` : ''}
            <tr><td style="padding:4px 8px;font-weight:bold;">Amount Due</td><td style="padding:4px 8px;color:#0a0a0a;font-weight:bold;">${totalStr}</td></tr>
            <tr><td style="padding:4px 8px;font-weight:bold;">Payment Terms</td><td style="padding:4px 8px;">Net 30 — due by ${dueDate}</td></tr>
          </table>
          <p style="margin-top:18px;">
            Please make checks payable to <strong>River City Roofing Solutions</strong>.
            Reference invoice number <strong>${invoice.invoiceId}</strong> on remittance.
          </p>
          <p>Questions? Reply to this email or call (256) 274-8530.</p>
        </div>
    ...
    if (!result.success) {
      console.error(
        `[pipeline] INVOICE_SENT email failed for ${invoice.invoiceId}:`,
        result.error
      );
    }
  }
  ...
  /**
   * Send the payment-receipt PDF email at PAYMENT_RECEIVED.
   *
   * PRICE ONLY. PDF marks the invoice as PAID. Customer receives it as
   * confirmation; admin cc'd for the office record.
   */
  private async _sendPaymentReceiptEmail(
    order: PipelineOrder,
    invoice: PipelineInvoice,
    ...
    if (!customerEmail) {
      console.warn(
        `[pipeline] PAYMENT_RECEIVED: no customer email on order ${order.orderId}; receipt not sent.`
      );
      return;
    }
    ...
    let pdfBuffer: Buffer;
    try {
      const { renderPaymentReceiptPDF } = await import('./invoice-pdf');
      pdfBuffer = await renderPaymentReceiptPDF(invoice, order, payment);
    } catch (err) {
      console.error('[pipeline] PAYMENT_RECEIVED: receipt PDF render failed:', err);
      return;
    }
    ...
    const body = `
      <div style="font-family: Arial, sans-serif; max-width: 640px; margin: 0 auto;">
        <div style="background: #0a0a0a; padding: 18px; text-align: center;">
          <h1 style="color: #39FF14; margin: 0; letter-spacing: 1px;">Payment Received</h1>
          <p style="color:#d1d5db;margin:6px 0 0;font-size:13px;">River City Roofing Solutions</p>
        </div>
        <div style="padding: 26px; background: #fff; color:#1f2937;">
          <p>Hi ${invoice.customerName || 'there'},</p>
          <p>
            We've received your payment. Thank you! Your receipt is attached
            as a PDF for your records.
          </p>
          <table style="width:100%;border-collapse:collapse;margin:14px 0;">
            <tr><td style="padding:4px 8px;font-weight:bold;">Invoice #</td><td style="padding:4px 8px;">${invoice.invoiceId}</td></tr>
            <tr><td style="padding:4px 8px;font-weight:bold;">Job</td><td style="padding:4px 8px;">${invoice.jobName || invoice.jobNumber || '—'}</td></tr>
            <tr><td style="padding:4px 8px;font-weight:bold;">Amount Received</td><td style="padding:4px 8px;color:#16a34a;font-weight:bold;">${amountStr}</td></tr>
            <tr><td style="padding:4px 8px;font-weight:bold;">Method</td><td style="padding:4px 8px;">${methodStr}</td></tr>
            ${payment.reference ? `<tr><td style="padding:4px 8px;font-weight:bold;">Reference #</td><td style="padding:4px 8px;">${payment.reference}</td></tr>` : ''}
          </table>
          <p>It was a pleasure working with you. Don't hesitate to reach out for any future roofing needs.</p>
        </div>
        <div style="background: #f5f5f5; padding: 14px; text-align: center; font-size: 12px; color: #666;">
          River City Roofing Solutions | (256) 274-8530 | rcrs@rivercityroofingsolutions.com
        </div>
    ...
    const filename = `receipt-${invoice.invoiceId}.pdf`;
    const result = await emailService.send({
      to: customerEmail,
      cc: ccList,
      subject,
    ...
    if (!result.success) {
      console.error(
        `[pipeline] PAYMENT_RECEIVED receipt email failed for ${invoice.invoiceId}:`,
        result.error
      );
    }
  }
