...
export interface InventoryItem {
  ...
  active: boolean;
  /**
   * Canonical stock unit (informational mirror of `unit`, but constrained
   * to a known set). Optional + backwards-compatible: existing rows without
   * this field default to undefined.
   *
   * The authoritative UoM-conversion logic lives in
   * `lib/normalize-line-item-qty.ts` keyed by productId — this field is
   * declarative metadata for UI display and downstream tooling.
   */
  unitOfMeasure?: 'each' | 'stick' | 'roll' | 'box' | 'bag' | 'bucket' | 'tube' | 'lb' | 'piece';
  /**
   * Linear feet covered per stock unit. Only set for SKUs sold by length
   * (e.g., Ridge Vent 4LF = 4). Used by the material-order parser to
   * convert PMs' LF entries into stick counts before persisting.
   */
