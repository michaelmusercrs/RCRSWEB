    ...
    <div className="min-h-screen bg-black text-white">
      ...
      {step === 1 && !photoMode && (
        <div className="max-w-xl mx-auto px-4 py-12">
          ...
          {/* Test Mode — runs 5 known addresses to calibrate AI methods */}
          <TestModeButton />
        </div>
      )}
...
// ══════════════════════════════════════════════════════════════════════════════
// CALIBRATION STATUS BADGE — shows in confidence section
// ══════════════════════════════════════════════════════════════════════════════

function CalibrationStatusBadge() {
  const [stats, setStats] = useState<{ totalCalibrations: number; methodAccuracy: Record<string, any> } | null>(null);

  useEffect(() => {
    fetch('/api/roof-measure/calibrate')
      .then(r => r.ok ? r.json() : null)
      .then(data => { if (data) setStats(data); })
      .catch(() => {});
  }, []);

  if (!stats || stats.totalCalibrations === 0) return null;

  const methods = Object.entries(stats.methodAccuracy);
  const reliable = methods.filter(([, s]) => s.reliable).length;

  return (
    <div className="mt-3 pt-3 border-t border-gray-700/50">
      <div className="flex items-center gap-2 mb-2">
        <BarChart3 className="w-3.5 h-3.5 text-[#39FF14]" />
        <span className="text-xs font-medium text-[#39FF14]">Calibrated</span>
        <span className="text-xs text-gray-500">
          {stats.totalCalibrations} ground truth sample{stats.totalCalibrations !== 1 ? 's' : ''} · {reliable}/{methods.length} methods reliable
        </span>
      </div>
        ...
        {methods.map(([method, s]) => (
          <span
            ...
            className={`text-[10px] px-1.5 py-0.5 rounded ${
              s.reliable ? 'bg-emerald-900/30 text-emerald-400' : 'bg-red-900/30 text-red-400'
            }`}
          >
            {method} ({(s.avgError * 100).toFixed(0)}% err)
          </span>
        ))}
      </div>
    </div>
...
// ══════════════════════════════════════════════════════════════════════════════
// TEST MODE: Run 5 known addresses to calibrate AI methods
// ══════════════════════════════════════════════════════════════════════════════
...
function TestModeButton() {
  ...
  const runTest = async () => {
    ...
    setReport(null);
    try {
      const res = await fetch('/api/roof-measure/test-mode', { method: 'POST' });
      if (!res.ok) throw new Error(`Test mode failed: ${res.status}`);
      const data = await res.json();
      setReport(data);
    } catch (e: any) {
      setError(e.message || 'Test mode failed');
    } finally {
      setRunning(false);
    }
  };
  ...
  return (
    <div className="mt-8 border-t border-gray-800 pt-6">
      <div className="flex items-center justify-between mb-3">
        <div>
          <p className="text-sm font-medium text-gray-300">🧪 Test Mode — AI Calibration</p>
          <p className="text-xs text-gray-500">Runs 5 known addresses to score AI method reliability</p>
        </div>
        <button
          onClick={runTest}
          disabled={running}
          className="px-4 py-2 bg-gray-800 border border-gray-700 text-gray-300 text-sm rounded-lg hover:bg-gray-700 transition flex items-center gap-1.5 disabled:opacity-50"
        >
          {running ? (
            <><Loader2 className="w-4 h-4 animate-spin" /> Running (~5 min)...</>
          ) : (
            <><Gauge className="w-4 h-4" /> Run Test Mode</>
          )}
        </button>
      </div>
      ...
      {report && (
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-3">
          <div className="flex items-center gap-3 text-sm">
            ...
            <span className="text-white font-mono">{report.summary?.addressesSucceeded}/{report.summary?.addressesTested}</span>
            <span className="text-gray-400 ml-4">Methods:</span>
            <span className="text-white font-mono">{report.summary?.methodsAnalyzed}</span>
          </div>

          {report.summary?.reliableMethods?.length > 0 && (
            <div className="flex flex-wrap gap-2">
              <span className="text-xs text-gray-500">Reliable:</span>
              {report.summary.reliableMethods.map((m: string) => (
                <span key={m} className="text-xs bg-emerald-900/30 text-emerald-400 px-2 py-0.5 rounded">{m}</span>
              ))}
            </div>
          )}

          {report.summary?.unreliableMethods?.length > 0 && (
            <div className="flex flex-wrap gap-2">
              <span className="text-xs text-gray-500">Unreliable:</span>
              {report.summary.unreliableMethods.map((m: string) => (
                <span key={m} className="text-xs bg-red-900/30 text-red-400 px-2 py-0.5 rounded">{m}</span>
              ))}
            </div>
          )}
          ...
          <details className="text-xs">
            <summary className="text-gray-500 cursor-pointer hover:text-gray-300">Detailed results...</summary>
            <div className="mt-2 space-y-2">
              {report.addresses?.map((a: any, i: number) => (
                <div key={i} className="bg-gray-800/50 rounded-lg p-2">
                  <p className="text-gray-300 font-medium">{a.address}</p>
                  {a.error ? (
                    <p className="text-red-400">{a.error}</p>
                  ) : (
                    <div className="mt-1 space-y-0.5">
                      {a.methods?.map((m: any, j: number) => (
                        <p key={j} className="text-gray-400 font-mono">
                          {m.method}: R{m.ridge} Rk{m.rake} E{m.eave} V{m.valley} H{m.hip}
                        </p>
                      ))}
                    </div>
                  )}
          ...
          {report.methodConsistency && Object.keys(report.methodConsistency).length > 0 && (
            <details className="text-xs">
              <summary className="text-gray-500 cursor-pointer hover:text-gray-300">Method consistency scores...</summary>
              <div className="mt-2 overflow-x-auto">
                <table className="w-full text-xs">
                  <thead><tr className="text-gray-500"><th className="text-left pr-3">Method</th><th className="text-right pr-3">Consistency</th><th className="text-right">Reliable</th></tr></thead>
                  <tbody>
                    {Object.entries(report.methodConsistency).map(([method, stats]: [string, any]) => (
                      <tr key={method} className="border-t border-gray-800/50">
                        <td className="py-1 pr-3 text-gray-300">{method}</td>
                        <td className="py-1 pr-3 text-right font-mono">{(stats.consistency * 100).toFixed(0)}%</td>
                        <td className={`py-1 text-right ${stats.reliable ? 'text-emerald-400' : 'text-red-400'}`}>
                          {stats.reliable ? '✓' : '✗'}
                        </td>
                      </tr>
                    ))}
...
// ══════════════════════════════════════════════════════════════════════════════
// CALIBRATION: Save Ground Truth Button
// ══════════════════════════════════════════════════════════════════════════════

function SaveGroundTruthButton({
  address, structId, manualOverrides, getFinalValue, measurementRows, report,
}: {
  address: string;
  structId: string;
  ...
  measurementRows: { key: string; label: string; ai: number }[];
  report: ApiResponse;
}) {
  ...
  const handleSave = async () => {
    ...
    setError('');
    try {
      // Build measurements from final values (manual overrides take precedence)
      const measurements: Record<string, number> = {};
      for (const row of measurementRows) {
        const final = getFinalValue(structId, row.key, row.ai);
        measurements[row.key] = final.value;
      }

      // Build AI results from pipeline data for correction factor calculation
      const aiResults = (report.pipeline?.allProviderResults || []).map((r: any) => ({
        method: r.method || r.name,
        provider: r.provider || 'unknown',
        model: r.model || 'unknown',
        imageSet: r.imageSet || 'unknown',
      ...
      const res = await fetch('/api/roof-measure/calibrate', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          source: 'manual',
          aiResults,
          roofArea_sqft: report.solarData?.totalRoofAreaSqFt,
        }),
      });
  ...
  if (saved) {
    return (
      <div className="flex items-center gap-2 text-[#39FF14] text-sm">
        ...
        <span>Calibration saved — future measurements will be more accurate</span>
      </div>
    );
  }
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <button
        onClick={handleSave}
        disabled={saving}
        className="px-4 py-2 bg-[#39FF14] text-black text-sm font-bold rounded-lg hover:bg-[#39FF14]/90 transition flex items-center gap-1.5 disabled:opacity-50"
      >
...
function ResultsView({
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6 pb-20">
      ...
      {/* SAVE AS GROUND TRUTH — feeds back into calibration */}
      {Object.keys(manualOverrides[structId] || {}).some(k => manualOverrides[structId][k]) && (
        <div className="bg-gray-900 border border-[#39FF14]/30 rounded-xl p-4 mb-4 no-print">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-[#39FF14]">Manual overrides detected</p>
              <p className="text-xs text-gray-400">Save these as ground truth to improve future AI measurements</p>
            </div>
            <SaveGroundTruthButton
