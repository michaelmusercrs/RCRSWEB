...
import { Plane, Trophy, Calendar, RefreshCw, Save, CheckCircle2, AlertCircle } from 'lucide-react';
...
export default function AwardsTripStandalone() {
  ...
  const load = useCallback(async (periodName?: string) => {
    ...
    setError(null);
    try {
      const params = new URLSearchParams({ presentMode: 'true' });
      if (periodName) params.set('period', periodName);
      const res = await fetch(`/api/command-center/awards-trip?${params.toString()}`);
  ...
  const save = async () => {
    ...
    try {
      ...
      for (const month of months) {
        const res = await fetch('/api/command-center/awards-trip?presentMode=true', {
          method: 'POST',
  ...
  if (!data) {
    return (
      <div style={{ minHeight: '100vh', background: '#09090b', color: 'white', padding: 32 }}>
        <p style={{ color: '#f87171' }}>{error || 'No data'}</p>
      </div>
    );
  }
